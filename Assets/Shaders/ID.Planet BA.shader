...
Shader "ID-Space-Shader/Planet BA" {
	Properties {
...
/////// Normals:
				float2 node_1985 = i.uv0;
				float3 normalLocal = UnpackNormal(tex2D(_NormalMap,TRANSFORM_TEX(node_1985.rg, _NormalMap))).rgb;
...
////// Emissive:
				...
				//float4 node_8 = tex2D(_DiffuseMap,TRANSFORM_TEX(node_1985.rg, _DiffuseMap));
				
				float4 adjustedDiffuseColor = tex2D(_DiffuseMap,TRANSFORM_TEX(node_1985.rg, _DiffuseMap));
				float4 node_8 = adjustedDiffuseColor * _Color;//tex2D(adjustedDiffuseColor,TRANSFORM_TEX(node_1985.rg, adjustedDiffuseColor));
