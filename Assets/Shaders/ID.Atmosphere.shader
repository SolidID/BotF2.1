...
Shader "ID-Space-Shader/Atmosphere" {
    Properties {
        _Diffuse ("Diffuse", 2D) = "white" {}
		_DiffuseColorAdjustment ("Diffuse Color adjustment", Color) = (1,1,1,1)
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform float4 _DiffuseColorAdjustment;
...
////// Lighting:
////// Emissive:
                float2 node_1543 = i.uv0;
				float3 currentColor = tex2D(_Diffuse,TRANSFORM_TEX(node_1543.rg, _Diffuse)).rgb;
				float3 someCalc = (_Color.rgb*_FallOff*pow(saturate(dot(i.normalDir,viewDirection)),3.0) * (currentColor * _DiffuseColorAdjustment));
				float3 someOtherCalc = lerp( pow(max(0,dot(i.normalDir,lightDirection)),1.5), 0.0, _FullBright );
                float3 node_1511 = (someCalc - someOtherCalc);
                float3 finalColor = node_1511 + node_1511;
/// Final Color:
